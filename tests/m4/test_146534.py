...
try:
    ...
    def TestCase():
      ...
      try:
          ...
          s_key = random.randint(0, 50)
          while s_key in keys:
            s_key = random.randint(0, 50)
    
          msg += "\n\nSearching for non-existent key: " + str(s_key)
...
f = open("/outputs/146534.json", "w")
json.dump({"id": "146534", "passed": output.passed, "log": output.logs}, f)
f.close()
