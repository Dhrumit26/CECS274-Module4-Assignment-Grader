...
function TestOutput {
    ...
    printf "$JSON_FMT" "146539" $1 "$logs"  > ../outputs/146539.txt
}
...
# Check if calc_in_1.txt exists
if [ ! -f "calc_in_1.txt" ]; then
    error_exit "calc_in_1.txt not found."
fi


# Run Python solution script and capture output
expected_output=$(python mainCP.py < calc_in_1.txt) || error_exit "Python solution script failed."
student_output=$(python main.py < calc_in_1.txt) || error_exit "Python script main.py failed."
...
echo "Testing Calculator System: Storing variables and values"
...
# Construct message
msg="Testing Calculator store_var(var, val, False) ...
